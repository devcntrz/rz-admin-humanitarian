...
export async function POST(
  ...
  return withCors(async (req) => {
    try {
      ...
      const siteReportId = parseInt(resolvedParams.id, 10)
      if (Number.isNaN(siteReportId)) {
      ...
      const parent = await sql`
        SELECT id FROM site_reports WHERE id = ${siteReportId} LIMIT 1
      `
      if (parent.length === 0) {
        return NextResponse.json(
      ...
      const result = await sql`
        ...
        VALUES (${siteReportId}, ${fileUrl}, ${description})
      ...
      console.error('documents_insert site-report:', error)
      return NextResponse.json(
